import pyautogui
import cv2
import numpy as np
import time
import keyboard
import json
from pynput import mouse
12
class ImageWatcher:

    def __init__(self, reference_image_path, region, key, threshold=0.35):
        self.reference_image = cv2.imread(reference_image_path, cv2.IMREAD_GRAYSCALE)
        self.region = tuple(region)
        self.key = key
        self.threshold = threshold

    def check_image(self):
        # Take a screenshot of the specified region and convert it to grayscale
        screenshot = pyautogui.screenshot(region=self.region)
        screenshot.save(f'{self.key}_screenshot.png')  # Save the screenshot for debugging
        screenshot = cv2.cvtColor(np.array(screenshot), cv2.COLOR_RGB2GRAY)

        # Apply template Matching
        print("Screenshot size: ", screenshot.shape)
        print("Reference image size: ", self.reference_image.shape)

        res = cv2.matchTemplate(screenshot, self.reference_image, cv2.TM_CCOEFF_NORMED)
        loc = np.where(res >= self.threshold)

        # If a match is found, press the key
        if len(loc[0]) > 0:
            pyautogui.press(self.key)
            return True
        return False

# Load the configuration from the JSON file
with open('config.json') as f:
    config = json.load(f)

# Create ImageWatcher instances for each set of parameters
watchers = [ImageWatcher(item['image_path'], item['region'], item['key']) for item in config]

# Start off with the script enabled
enabled = [True]

def toggle_enabled(e):
    enabled[0] = not enabled[0]
    print(f"Script is now {'enabled' if enabled[0] else 'disabled'}")

def press_two(x, y, button, pressed):
    if enabled[0] and button == mouse.Button.left and pressed:
        pyautogui.press('2')

# Start listening for the numlock key to toggle the script
keyboard.on_press_key('numlock', toggle_enabled)
mouse_listener = mouse.Listener(on_click=press_two)
mouse_listener.start()

while True:
    if enabled[0]:
        # Continuously check the region for the reference image and press the key if found
        # But only if the region has been set
        for watcher in watchers:
            print(f'Checking {watcher.region}')
            if watcher.region is not None and watcher.check_image():
                print(f'Pressed {watcher.key} for {watcher.region}')
                # If a key was pressed, break the loop and start checking the next watcher1
            else:
                print(f'No match for {watcher.region}')
    time.sleep(0.1)  # Wait a little between each check to not use 100% CPU111